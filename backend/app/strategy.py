...
def analyze(session: Session) -> Analysis:
    ...
    if fear_level < FNG_STRONG_FEAR:
        score += 3
        factors.append(f"F&G {fear_level} ({fng['classification']}, +3)")
    elif fear_level < FNG_FEAR:
        score += 2
        factors.append(f"F&G {fear_level} ({fng['classification']}, +2)")
    elif fear_level < FNG_NEUTRAL:
        factors.append(f"F&G {fear_level} ({fng['classification']}, +0)")
    else:
        score -= 2
        factors.append(f"F&G {fear_level} ({fng['classification']}, -2)")
